...
- **Retrieval ≠ citation — optimize both.** Retrieval (appearing in the model's search results) is won by title/H2 token match. Citation (the visible credit + click) is won at the snippet level: models attribute a fact to the snippet they lifted it from. Rules that follow: (1) every key fact must appear in a plain prose sentence, not only inside a table — snippets extract prose, and a fact that lives only in a table cell gets cited to whichever competitor stated it in a sentence; (2) the first 1-2 sentences under each H2 must answer self-containedly (existing quotable-answers rule — this is why it exists); (3) at least one genuinely original claim or angle per post that no other source has — commodity facts are shared with five other sites, unique facts force the citation to us.
